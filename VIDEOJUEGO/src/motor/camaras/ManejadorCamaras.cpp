...
void ManejadorCamaras::unlock_objeto()
{
...
void ManejadorCamaras::renderizar(std::vector<Enemy*> objetos)
{
    std::vector<Objeto*> aux;
    for(auto &p:objetos)
    {
        aux.push_back(p);
        //printf("%d,%d\n",p->get_posicion_camara().x,p->get_posicion_camara().y);
        //printf("%d,%d\n",p->get_posicion_mundo().x,p->get_posicion_mundo().y);
    }
    get().camara_actual->renderizar(aux);
};

void ManejadorCamaras::update(std::vector<Enemy*> objetos)
{
    std::vector<Objeto*> aux;
    for(auto &p:objetos)
    {
    ...
    get().camara_actual->update();
    get().camara_actual->proyectar(aux);
};